...
class _MiMapaState extends State<MiMapa> { 
  ...
  Future<void> updateScreenCoordinates() async {
    ...
    Offset offsetmedioBtemp    = await getScreenPosition(makerB);
      setState(() {
  ...
  void setInitialData ()  {
     ...
     makerA = calcularPuntoMedio(puntoA, puntoMedio);
     makerB= calcularPuntoMedio(puntoMedio, puntoB);
     ...
     salida = Position(
      ...
      //inicializamos la position del punto medio  para calcular las distacias luego
      pMedio = Position(
 ...
 double calcularBearing(LatLng start, LatLng end) {
  ...
  void _calculateBering() {
    ...
    LatLng end = LatLng(widget.hoyo.hoyo.centroHoyo!.latitud, widget.hoyo.hoyo.centroHoyo!.longitud);
